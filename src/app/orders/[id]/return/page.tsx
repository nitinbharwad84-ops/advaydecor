...
export default function ReturnOrderPage() {
    ...
    useEffect(() => {
        if (!isAuthenticated) {
            router.push(`/login?redirect=${encodeURIComponent(`/orders/${orderId}/return`)}`);
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`/api/orders/${orderId}/return`, {
                ...
                body: JSON.stringify({ reason: reason.trim(), isPackaged, isUnused }),
            });
                    ...
                    >
                        {success ? (
                            <div style={{ textAlign: 'center', padding: '2rem 0' }}>
                                ...
                                <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#0a0a23', marginBottom: '0.75rem' }}>Return Requested</h2>
                                <p style={{ color: '#64648b', fontSize: '0.95rem', lineHeight: 1.7, marginBottom: '2rem', maxWidth: '380px', margin: '0 auto 2rem' }}>
                                    Your return request has been submitted. Our team will review the details and notify you regarding the next steps.
                                </p>
                            ...
                            <>
                                <div style={{ marginBottom: '1.75rem' }}>
                                    ...
                                    <p style={{ color: '#64648b', fontSize: '0.875rem', lineHeight: 1.6 }}>
                                        Provide the details below so our admin team can review and process your return.
                                    </p>
                                </div>
                                ...
                                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                                    ...
                                    {/* Reason */}
                                    <div>
                                        <label htmlFor="return-reason" style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, color: '#0a0a23', marginBottom: '0.5rem' }}>
                                        ...
                                        <textarea
                                            ...
                                            placeholder="e.g. Product arrived damaged, wrong item received, does not match the description..."
                                            rows={4}
                                            style={{
                                    ...
                                    {/* Condition Checklist */}
                                    <div style={{ background: '#f9f7f3', padding: '1.25rem', borderRadius: '0.75rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                        <p style={{ fontSize: '0.8rem', fontWeight: 600, color: '#64648b', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Product Condition</p>
                                        ...
                                        <label style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', cursor: 'pointer' }}>
                                            <input
                                                ...
                                                style={{ width: '1.15rem', height: '1.15rem', accentColor: '#0a0a23', cursor: 'pointer' }}
                                            />
                                    ...
                                    >
                                        {submitting ? <Loader2 size={20} className="animate-spin" /> : 'Submit Return Request'}
                                    </button>
                                </form>
                            </>
                        )}
