...
import { Search, Eye, ChevronDown, XCircle } from 'lucide-react';
import toast from 'react-hot-toast';

const statusColors: Record<string, { bg: string; text: string }> = {
    'Cancelled': { bg: 'rgba(239, 68, 68, 0.1)', text: '#dc2626' },
    'Cancellation Requested': { bg: 'rgba(245, 158, 11, 0.1)', text: '#d97706' },
};
        ...
        fetch('/api/admin/orders')
            ...
            .then(data => {
                if (Array.isArray(data)) {
                    setOrders(data.filter(o => o.status === 'Cancelled' || o.status === 'Cancellation Requested'));
                }
                setLoading(false);
            })
    ...
    return (
        <div style={{ maxWidth: '1200px' }}>
            ...
            <div style={{ marginBottom: '2rem' }}>
                ...
                <p style={{ fontSize: '0.875rem', color: '#9e9eb8', marginTop: '0.25rem' }}>Manage cancelled orders and requests ({orders.length} total)</p>
            </div>
            ...
            >
                                            ...
                                            <td colSpan={8} style={{ padding: '1.25rem 1.5rem', background: 'rgba(245,240,232,0.3)' }}>
                                                <div className="admin-order-detail-grid">
                                                    ...
                                                    <div style={{ gridColumn: '1 / -1', background: '#fff', padding: '1rem', borderRadius: '0.5rem', border: '1px solid #f0ece4', marginBottom: '1rem' }}>
                                                        <h4 style={{ fontSize: '0.75rem', fontWeight: 600, color: '#dc2626', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Cancellation Reason</h4>
                                                        <p style={{ fontSize: '0.875rem', color: '#0a0a23' }}>{order.cancel_reason || 'No reason provided by the user.'}</p>
                                                        ...
                                                        {order.status === 'Cancellation Requested' && (
                                                            <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                                                                <button
                                                                    ...
                                                                    style={{ padding: '0.5rem 1rem', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '0.5rem', fontSize: '0.875rem', fontWeight: 500, cursor: 'pointer', opacity: actionLoading === order.id ? 0.7 : 1 }}
                                                                >
                                                                    {actionLoading === order.id ? 'Processing...' : 'Approve Cancellation'}
                                                                </button>
                                                                <button
                                                    ...
                                                    {/* Order Items */}
                                                    <div>
                                                        <h4 style={{ fontSize: '0.7rem', fontWeight: 600, color: '#9e9eb8', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Order Items (Cancelled)</h4>
                ...
                {filteredOrders.length === 0 && (
                    <div style={{ textAlign: 'center', padding: '3rem 0' }}>
                        <XCircle size={40} style={{ margin: '0 auto', color: '#9e9eb8', marginBottom: '1rem' }} />
                        <p style={{ color: '#64648b' }}>No cancelled orders found</p>
                    </div>
                )}
            </motion.div>
